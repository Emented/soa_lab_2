...
@EnableWs
@Configuration
class WebConfig : WsConfigurerAdapter() {
    ...
    @Bean(name = ["main"])
    fun defaultWsdl11Definition(
        mainSchema: XsdSchema
    ): DefaultWsdl11Definition {
        val wsdl11Definition = DefaultWsdl11Definition()
        wsdl11Definition.setPortTypeName("MainPort")
        wsdl11Definition.setLocationUri("/ws")
        wsdl11Definition.setTargetNamespace("http://emented/soa")
        wsdl11Definition.setSchema(mainSchema)
        return wsdl11Definition
    }

    @Bean
    fun mainSchema(): XsdSchema {
        return SimpleXsdSchema(ClassPathResource("main.xsd"))
    }
}